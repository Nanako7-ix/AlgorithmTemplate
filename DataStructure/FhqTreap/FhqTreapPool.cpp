...
struct FhqTreapPool {
	...
	void splitByValue(int u, T k, int& x, int& y, int fx, int fy) {
		if (!(x = y = u)) return;
		if (val[u] <= k) {
			fa[u] = fx;
	...
	std::pair<int, int> splitByValue(int u, T k) {
		int x, y;
...
template<typename T>
std::mt19937_64 FhqTreapPool<T>::rng(std::chrono::steady_clock::now().time_since_epoch().count());
